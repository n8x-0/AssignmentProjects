...
let fromConvVal: string;
let toConvertVal: string
let amountVal: number;
let amBg = chalk.yellow('amount');
let highlightedText = chalk.yellow(`enter existing code `)

async function logApi(currency: string, amount: number, toconvert: string) {

    fetch(url + currency)
        .then(resp => resp.json())
        .then((data) => {
            let rJson = Object.entries(data.data)
            let filteredItems: any = rJson.filter((item: any) => item[1].code === toconvert);
...
async function startConvertor(par: any) {
